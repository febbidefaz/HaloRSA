...
export default function RiwayatPendaftaran() {
  ...
  const [selectedRatingItem, setSelectedRatingItem] = useState<RiwayatReg | null>(null);
      ...
      [
        { text: 'Tidak', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await fetch(
                `http://app.rsabojonegoro.com:4000/his/about/deleteReg?regid=${selectedItem.idol}`,
                {
                  method: 'DELETE',
                }
              );
...
const disableBatal =
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      const res = await fetch(
        'http://app.rsabojonegoro.com:4000/his/reg/newRating',
        {
          method: 'POST',
          headers: {
        ...
        <View style={styles.modalBox}>
            <View style={styles.modalHeader}>
                ...
                <Text style={styles.disableInfo}>
                    Tidak dapat dibatalkan karena sudah lewat hari atau sudah diproses.
                </Text>
                )}
            ...
            <>
                ...
                <Text style={styles.detailValue}>{selectedItem.doctorname}</Text>
                </View>

                <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Pasien</Text>
                <Text style={styles.detailValue}>{selectedItem.patientname}</Text>
                </View>
                ...
                <Text style={styles.detailValue}>
                    {selectedItem.jawal?.slice(0, 5)} - {selectedItem.jakhir?.slice(0, 5)}
                </Text>
                </View>
            </>
            )}
